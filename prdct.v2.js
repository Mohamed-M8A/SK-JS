...
(function () {
  ...
  document.addEventListener("DOMContentLoaded", async function () {
    ...
    const feedUrl = `/feeds/posts/default/-/${encodeURIComponent(
      storeLabel
    ...
    try {
      ...
      if (!entry) {
        sellerBarContainer.innerHTML =
          "<p>⚠ لم يتم العثور على بيانات البائع.</p>";
        return;
      }
    ...
    try {
      ...
      function renderPage(page) {
        ...
        container.innerHTML = pageEntries
          .map((post) => generatePostHTML(post, true))
          .join("");
